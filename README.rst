...
原稿を HTML ファイル群に変換して上記 URL に配備するには、次のブランチのいずれかへの
``git push`` で発動する GitHub Actions ワークフローにビルド、配備させるものとする：

* ``master``
* ``develop``

このワークフローの処理時間はだいたい 3, 4 分で完了するようだ。

ワークフローの実際の処理についてはファイル ``.github/workflows/ci.yml`` を見ろ。
...
当プロジェクトは個人的な読書・学習ノートであるが、それにもかかわらず、快く貢献、協力、情報提供をくださったすばらしい
GitHub 使用者の一覧を
