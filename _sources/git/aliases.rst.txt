...
Git にもある。方法はファイル :file:`.gitconfig` での設定による。つまりコンソール
で次のようにタイプするか、
...
* 記法はテキストエディターで :file:`.gitconfig` に入力する文字列の等号の右辺をそ
  のまま記す。

エイリアスを設ける代わりに、各コマンドの構成オプションを設定することも検討した
い。例えばコマンド :command:`X` の挙動を常に :command:`Y` にしたければ、構成オプ
ション``X.Y`` のようなものが提供されているかどうかを調べる。
...
``diff = diff --word-diff``
  コマンド :command:`diff` で常に単語レベルでの差分を表示する。
...
``gr = grep -Ii``
  コマンド :command:`grep` を利用する際に、バイナリーファイルはコマンドの対象と
  しないならば ``-I`` を、大文字小文字を区別しないで検索するならば ``-i`` を指定
  する。
...
``lc = log ORIG_HEAD.. --stat --no-merges``
  前回 :command:`fetch` 以降のすべてのコミットを統計とともにリストする。ただし
  マージは除く。
...
``rh2 = reset HEAD^^ --hard``
  直近の 2 個のコミットをなかったことにして、作業コピーの状態をも戻す。ローカル
  での変更分が失われる。危険。

コマンド :command:`shortlog`
----------------------------------------------------------------------
