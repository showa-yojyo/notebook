...
Git の個人的な構成オプションはファイル :file:`$HOME/.gitconfig` に保存する。テキ
ストエディターで直接テキストを編集してもよいし、コンソールから ``git config
--global`` で設定を書き込んでもよい。
...
コマンド ``git config`` が検索する設定ファイルパスのうち、ある種の環境変数による
設定を考慮しなければ、個人設定に関係するものは次のとおり：

1. :file:`$XDG_CONFIG_HOME/git/config`
2. :file:`$HOME/.config/git/config`
3. :file:`$HOME/.gitconfig`

Git のバージョンが古い場合に備えて 3. に一本化しておくのが無難だが、そうすると設
定ファイル各種をバージョン管理する都合があるので不便なこともある。
