...
:program:`conda` でインストール
----------------------------------------------------------------------
Python 環境を Anaconda_ もしくは Miniconda_ を利用して管理しているのであれば、
コンソールで :code:`conda install pygments` とすればパッケージがインストールできる。

* :program:`conda` については :doc:`/python-miniconda` 参照。

:program:`pip` でインストール
----------------------------------------------------------------------
Python 環境を正規のインストーラーから構築しているのであれば pip_ が利用可能だ。
コンソールで ``pip install pygments`` とする。

* :program:`pip` については :doc:`/python-pip` 参照。
