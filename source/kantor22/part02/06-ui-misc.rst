...
| Method                   | Description                               |
+==========================+===========================================+
| ``                       | 現在の選択範囲を ``node`` の ``offset``   |
| collapse(node, offset)`` | 位置を指す空範囲とする                    |
+--------------------------+-------------------------------------------+
| ``set                    | 同上                                      |
| Position(node, offset)`` |                                           |
+--------------------------+-------------------------------------------+
| ``collapseToStart()``    | 選択を                                    |
|                          | 現在のそれの開始位置にある空範囲に縮める  |
+--------------------------+-------------------------------------------+
| ``collapseToEnd()``      | 上の終了位置版                            |
+--------------------------+-------------------------------------------+
| ``extend(node, offset)`` | 選択の focus を ``node`` の ``offset``    |
|                          | に動かす                                  |
+--------------------------+-------------------------------------------+
| ``setBaseAndExtent(anc   | いちばん詳細な指定で選択                  |
| horNode, anchorOffset, f |                                           |
| ocusNode, focusOffset)`` |                                           |
+--------------------------+-------------------------------------------+
| ``s                      | ``node`` の子すべてを選択する             |
| electAllChildren(node)`` |                                           |
+--------------------------+-------------------------------------------+
| ``deleteFromDocument()`` | 内容をページから削除する                  |
+--------------------------+-------------------------------------------+
| ``conta                  | ``node`` を含んでいるか                   |
| insNode(node, allowParti |                                           |
| alContainment = false)`` |                                           |
+--------------------------+-------------------------------------------+
...
|                                  | トボックスにあるすべてを選択する |
+----------------------------------+----------------------------------+
| ``setSelection                   | テキストを選択する               |
| Range(start, end, [direction])`` |                                  |
+----------------------------------+----------------------------------+
| ``setRangeText(replacement, [s   | テキスト範囲を置換する           |
| tart], [end], [selectionMode])`` |                                  |
+----------------------------------+----------------------------------+
