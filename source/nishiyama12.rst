...
* :math:`\mathbb{P}^1` は :math:`S^1` の半分と同型。無限遠点一つのみ。
* :math:`\mathbb{P}^2` は :math:`S^2` の半分と同型。円周が無限遠点。
