...
インストール方法
======================================================================
* インターネットが利用できる環境では、選択肢はふたつある。

  * Anaconda_ または Miniconda_ で環境を管理していれば、
    コマンドライン :code:`conda install jinja2` でよい。

  * 正規の Python インストーラーで環境を構築していれば、
    :code:`pip install jinja2` でよい。

  * いずれの場合でも先に Sphinx をインストールしてしまうのがなおよい。
    そうすることで依存パッケージである Jinja2 もインストールされる。

* インターネットが利用できない環境では、
  できる環境から圧縮ファイルを持ち帰り、解凍してがんばる。

  * 解凍して :file:`setup.py` のあるフォルダーで :code:`python setup.py install` とする。
  * whl 形式のファイルが入手できるならば、
    :code:`pip install jinja2-x.y.z.whl` でよい。
