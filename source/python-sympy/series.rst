...
関数 :code:`limit(e, z, z0, dir='+')`
  変数 :code:`z` が :code:`z0` に近づくときの関数 :code:`e(z)` の極限値を求める。

  * 引数 :code:`e` は SymPy オブジェクト
  * 引数 :code:`z` は :code:`e` の極限を考える変数。
  * 引数 :code:`z0` は :code:`z` を何に近づけるかを値で指定する。
  * キーワード引数 :code:`dir` で右極限か左極限を指定する。プラスが右。

クラス Limit
  関数版と同じことをするが、いわば遅延評価版。
  実際に極限を求める処理を実行するには、メソッド :code:`doit(**hint)` を呼び出す。
...
ここには関数 :code:`series` がある。

関数 :code:`series(expr, x=None, x0=0, n=6, dir='+')`
  関数 :code:`expr` の点 :code:`x = x0` の周りでの :code:`n` 次のべき級数展開を求める。
...
* 各出力の :code:`n` 次以上の項にある :code:`O(x)` については次節に記す。
...
クラス Order
  ...
  * デフォルトでは変数が 0 に近づくときの極限の評価を与える。
    無限大の評価をする場合は明示的に変数と :code:`oo` とのペアを与える必要がある。

  * 実はエイリアス :code:`O = Order` が定義されている。
    当然これを積極的に利用する。

  * :code:`O(f(x), x)` は内部で自動的に
    :code:`O(f(x).as_leading_term(x), x)` と変形されるとのこと。
    これは一変数の場合で、それ以上の多変数の場合は
    この規則がパラメーターのシンボル順に適用される。

  * ビッグオー記法オブジェクトは概念としては集合なので、
    同じ変数と関係する関数と :code:`Order` オブジェクトに対しては
    演算子 :code:`in` が適用できる。
...
* 上の例の :code:`lg` の定義はこのデモにとっては必要ない。
...
関数 :code:`residue(expr, x, x0)`
  関数 :code:`expr` の点 :code:`x = x0` における留数を求める。
...
* [5] の計算は誤りだ。留数として ``-1`` が出力されて欲しい。
  ...
  * ちなみに :code:`zoo` は :code:`S.ComplexInifinity` の意。
...
関数 :code:`sequence(seq, limits=None)`
  ...
  * キーワード引数 :code:`limits` は数列の定義域を指定するために使う。

    * デフォルトは区間 :code:`[0, oo)` 上の添字が許される数列を生成する。
    * 有限数列でも無限数列でも構わないが、
      整数全体を区間として指定することはできない。
      例外 ValueError が送出する。
...
* クラス EmptySequence は空の数列を表現するためのもの。
  オブジェクトを :code:`S.EmptySequence` として参照できる。
...
* :code:`gen`: 数列の生成要素。状況によって肩が異なる。
* :code:`interval`, :code:`start`, :code:`stop`, :code:`length`: 数列の定義域に関する情報。
* :code:`variables`: 数列の添字シンボル。
  一般に多重数列であることもあるので tuple 型である。

また、数列オブジェクトに対して次のような演算子が使える。

* 単項演算子 :code:`-`
* 二項演算子 :code:`+=`, :code:`-=`, :code:`*=`, :code:`+`, :code:`-`, :code:`*`
...
クラス FourierSeries
  ...
  * 与えられた関数を :code:`f` とした。
  * 最初の引数 :code:`self` は省略した。
  * :code:`s` は変数 :code:`x` と独立な項とする。
