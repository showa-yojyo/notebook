...
関数 :code:`div(f, g, *gens, **kwargs)`
  ...
  * 戻り値は tuple オブジェクトで、先頭から順に商、余りだ。
    ...
    * キーワード引数 ``polys=True`` を指定すると、
      商、余りを表現する Poly オブジェクトそれぞれを要素とする
      tuple オブジェクトを得られる。

  * キーワード引数 :code:`domain` で多項式の係数をどの体で
    考えるのかを指定することが可能だ。
    値として文字列 ``'ZZ'`` や ``'QQ'`` などを指定する。
    例を示す：

    .. code:: ipython

       In [1]: div(x**5 + 4*x**2 + 9, 2*x**3 - 7*x, domain='QQ')
       Out[1]: (x**2/2 + 7/4, 4*x**2 + 49*x/4 + 9)

       In [2]: div(x**5 + 4*x**2 + 9, 2*x**3 - 7*x, domain='ZZ')
       Out[2]: (0, x**5 + 4*x**2 + 9)

       In [3]: div(x**5 + 4*x**2 + 9, 2*x**3 - 7*x, domain='RR')
       Out[3]: (0.5*x**2 + 1.75, 4.0*x**2 + 12.25*x + 9.0)

       In [4]: div(x**5 + 4*x**2 + 9, 2*x**3 - 7*x)
       Out[4]: (x**2/2 + 7/4, 4*x**2 + 49*x/4 + 9)

  * 多変数多項式にも対応している。
    どのシンボルが変数なのかを明示的に指示する方法があるらしい。

  * :code:`f` と :code:`g` を両方定数にすると、
    例外 ComputationFailed を送出することで計算を失敗として処理する。
