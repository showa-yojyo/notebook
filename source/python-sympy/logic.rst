...
* クラス Boolean のスーパークラスはクラス Basic である。
  このクラスが論理演算子 :code:`&`, :code:`|`, :code:`~`, etc. をオーバーロードしているので、
  オブジェクト :code:`A`, :code:`B` がある要件を満たしていれば、
  例えば :code:`A & B` のような記法で論理式オブジェクトを生成できる。

* クラス BooleanTrue と BooleanFalse は Singleton のサブクラスでもある。
  いつものようにオブジェクト :code:`S` を用いて、それぞれのオブジェクトにアクセスできる。
...
.. csv-table::
   ...
   And@:code:`&`@論理積、かつ、conjunction
   Or@:code:`|`@論理和、または、[inclusive] disjunction
   Not@:code:`~`@否定、でない、negation
   Xor@:code:`^`@排他的論理和、exclusive disjunction
...
論理演算の基本は :code:`&`, :code:`|`, :code:`~` である。
それぞれに対応するクラスが存在し、名前は And, Or, Not である。
SymPy ではシンボルや論理式オブジェクトをこれらの組み合わせで組み立てることで表現する。

* メソッド :code:`as_set` で具体的な集合オブジェクトを生成する。
  例えば :code:`And(-2 < x, x < 2).as_set()` は :code:`Interval(-2, 2)` を生成する。

  * ここで :code:`-2 < x` 等はまた別の型だが、今は気にしない。

* 論理式オブジェクトでもメソッド :code:`subs` を利用できる。
...
真理値表 :code:`minterms` はこのように入れ子の list で表現する。
例えば :code:`[0, 0, 1]` の意図は「命題 :code:`~x & ~y & z` が真である」だ。

次に `Wikipedia の例 <https://en.wikipedia.org/wiki/Quine%E2%80%93McCluskey_algorithm>`_ を再現してみよう。
ここでは引数 :code:`dontcare` も指定する。
...
* さらに f が Don't care な行は :code:`minterms` では含めない。
  むしろ :code:`dontcare` の入れ子要素にする。

:code:`Out[2]` が当該記事の最終結果と一致している。
そして :code:`Out[2]` と :code:`Out[3]` は等値だ。
...
関数 :code:`to_cnf(expr, simplify=False)`, :code:`is_cnf(expr)`
  論理式オブジェクト :code:`expr` から、等値性を保ったまま
...
関数 :code:`simplify_logic(expr, form=None, deep=True)`
  この関数は論理式 :code:`expr` を単純化したオブジェクトを生成する。

  * キーワード引数 :code:`form` に値を指定するならば、
    文字列 :code:`cnf` または :code:`dnf` を与えること。
    その値に対応する形式の論理式オブジェクトが得られる。

  * キーワード引数 :code:`deep` は単純化の再帰をするかどうかを示すフラグ。

汎用の単純化関数 :code:`simplify` も論理式オブジェクトを単純化するのに使える。
...
関数 :code:`satisfiable(expr, algorithm='dpll2', all_models=False)`
  論理式 :code:`expr` の充足性をテストする。

  * 論理式 :code:`expr` が真になる可能性が全くなければ False を返す。
  * 論理式 :code:`expr` が真になる可能性があれば、
  ...
  * キーワード引数 :code:`all_models=True` を指定すると、
