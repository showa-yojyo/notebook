...
* 一部のサブクラスは ``from sympy.sets.fancysets import ...`` しないと利用可能にならない。
...
``is_proper_subset``
  ``E`` が ``T`` の部分集合であり、かつ ``E != T`` であるかを返す。

``is_proper_superset``
  ``is_proper_subset`` を逆方向に試す。

``contains``
  これは引数が集合ではなく、何かの値 ``x`` である。
  つまり ``x`` が ``E`` の元であるかを返す。
...
* コンストラクターでオブジェクトを生成することも、
  シングルトン管理オブジェクトから ``S.EmptySet`` として参照することもできる。
  クラス ``EmptySet`` がシングルトンゆえ、両者は同一のオブジェクトだ。

* 関数 ``len`` を ``EmptySet`` オブジェクトに適用すると常に ``0`` を返す。
* ルベーグ測度が常に ``0`` である。
* メソッド ``contains`` は常に SymPy の ``false`` を返す。
* メソッド ``intersection`` は常に空集合オブジェクトを返す。
* メソッド ``union``, ``complement``, ``symmetric_difference`` は常に引数の集合オブジェクト自身を返す。
...
* クラス ``EmptySet`` 同様、シングルトンである。
  シングルトン管理オブジェクトから ``S.UniversalSet`` として参照できる。

* 関数 ``len`` は利用不可。
* ルベーグ測度が常に ``S.Inifinity`` である。
* メソッド ``boundary`` は常に ``S.EmptySet`` を返す。
* メソッド ``contains`` は常に SymPy の ``true`` を返す。
...
* コンストラクターについて
  ...
  * サポートするキーワード引数は ``evaluate`` だけとなる。
    ...
    * キーワード引数 ``evaluate`` は Python のブーリアン値をとる。
      これが ``True`` のときには要素を簡略化する。
      つまり静的メソッド ``reduce`` が呼ばれる。
...
* オブジェクトが iterable である場合とそうでない場合がある。
  ...
  * アクセス順は Python 標準の関数 ``product`` を各集合に適用したものと同じらしい。
...
* コンストラクター
  ...
  * 呼び出し形式は ``Interval(start, end, left_open=False, right_open=False)`` である。
    キーワード引数の意味は、その名前が示す通り。

  * ``start < end`` となるように引数を指示しないと、
    空集合オブジェクトになってしまう。

  * ``start == end`` となるように指示すると、
    一点からなる ``FiniteSet`` オブジェクトが生成してしまう。

  * 端点として ``S.Infinity`` 等も指定できる。

  * メソッド ``Lopen``, ``Ropen`` 等により、
...
オブジェクトの生成は ``ImageSet(f, E)`` のようにすればよい。

* あるいは関数 ``imageset`` を用いて ``imageset(x, expr, E)`` のようにする。
* または ``imageset(lambda x: f(x), E)`` のような。
* 他にもある。
...
* ``R2 = S.Reals * S.Reals`` または ``R2 = S.Reals ** 2`` とすれば :math:`\mathbb{R}^2` を表現できる。

  * ``(0, 0) in R2`` のようなコードが書ける。

* ``Interval(0, 1, True, True) ** 2`` に対するメソッド ``boundary`` の戻り値がおもしろい。
