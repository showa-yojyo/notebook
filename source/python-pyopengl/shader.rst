...
コンストラクター
----------------------------------------------------------------------
* 諸般の事情により、コンテキストバージョンを 2.1 に決め打つ。
...
関数 ``glDeleteShader`` の呼び出しはここでよいのだが、場合によってはいつものメソッド ``cleanup`` まで先送りできる。
...
メソッド ``init_gl``
----------------------------------------------------------------------
背景色を白くしたいためだけに一応オーバーライドしたが、しなくても画面は描画される。

メソッド ``do_render``
----------------------------------------------------------------------
レガシー API 満載の実装につき、説明は差し控える。
