...
* この付録では UML 図の一般的な特徴および、
  それがどのように UML モデルとこの要素に関係するのかをを述べる。
...
* Figure A.1 UML Diagram
  ...
  * 図のそれぞれは contents area を持つ。
    オプションとして、枠と表題を持つことも認められる。
...
* Figure A.3 Two diagrams of packages
  ...
  * 左の図は
    ...
    図表の記号を持ったパッケージ ``CP`` を表すパッケージ図である。

  * 右の図はこのパッケージ ``P`` を表すクラス図である。

  * 補足事項だが、左図のパッケージ記号は
    クラス記号も関連記号も含む必要はない。
    より現実にありそうなモデルでは、パッケージ記号は
    通常パッケージの名前だけを持つ。
    一方、パッケージを表すクラス図はパッケージで定義されるクラスを
    表す記号を持つ。

* Figure A.4 A class diagram and a composite structure diagram

  * 左の図はパッケージ ``Cars`` を示すクラス図で、
    パッケージ ``Cars`` はクラス ``Car`` を含む
    ことを表現する記号を持つ。

  * 右の図はこのクラス ``Car`` を表す合成構造図である。

  * 左の図のクラス記号は区画内にクラスの構造を含める必要はない。
    より現実的なモデルでは、クラス記号は普通は単にクラス名を持つだけであるが、
    一方クラスを表す合成構造図は合成構造を表す記号を持つ。
...
* Figure A.5 The taxonomy of structure and behavior diagrams
  ...
  * 図の種類は構造図と振る舞い図の二種類に大別される。

* 構造図はシステム内のオブジェクトの静的な構造を示す。
  すなわち、時間を考慮しない仕様におけるそれらの要素を描写する。

* 振る舞い図はシステム内のオブジェクトの動的な振る舞いを示すもので、
