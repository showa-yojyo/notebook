...
リテラル、式、時間、範囲のようなカテゴリーに分けて説明している。

* リテラル (literal) の何か適切な日本語表現はあるだろうか。
...
----------------------------------------------------------------------
* Figure 8.1 によると、6 種類の LiteralSpecification の具象要素が存在する。
  次節で意味を仕様化する。
...
8.3.1 Summary
----------------------------------------------------------------------
Expression はある計算から導かれる値を指定する ValueSpecification である。

8.3.2 Abstract Syntax
----------------------------------------------------------------------
Figure 8.2 を見ると、2 系統の Expression が存在することがわかる。
...
StringExpression
  ...
  * Expression かつ TemplateableElement である。
...
A_operand_expression
  * Expression から ValueSpecification への関連。
  * A_ownedElement_owner の subsets 関連である。
  * 関連端 operand には制約 ``{ordered}`` がある。当然だ。
  * 木構造なので composite である。

A_subExpression_owningExpression
  * StringExpression は自身と同型要素の composite である。
    先ほどの関連と構造が同じである。

A_behavior_opaqueExpression, A_result_opaqueExpression
  * OpaqueExpression から Behavior への単方向関連。
  * この関連は OpaqueExpression::body がない場合に意味を持つ。
  * 関連端 result は制約 ``{readOnly}`` が課せられる。
...
----------------------------------------------------------------------
Figure 8.3 は一箇所を除いて左右対称なグラフとなっている。
...
*Observation*
  * 何か NamedElement に関係して起こるイベントの観測を示す PackageableElement である。
  * 興味のあるイベントとは、対象の NamedElement がいつ「入られた」または「出られた」かである。

TimeObservation
  * 特定の NamedElement に「入る」か「出る」かの一方を観測する Observation である。
  * 属性 firstEvent が true ならば「入る」であり、そうでなければ「出る」を観測する。
  * TimeObservation の結果とは、観測イベントの起こる時点である。

DurationObservation
  * 一つか二つどちらかの NamedElement に関係する期間を観測する Observation である。
...
TimeExpression, Duration
  * 特定の時点（期間）を評価する ValueSpecification である。
  * 多分何か与えられた観測要素の集合に関係する時点（期間）である。
    下記の対応関連のノート参照。

A_expr_timeExpression, A_expr_duration
  * それぞれ TimeExpression, Duration から ValueSpecification への composite な関連である。
  * 関連 A_ownedElement_owner の subsets な関連である。
...
A_observation_timeExpression, A_observation_duration
  * それぞれ TimeExpression, Duration から Observation への関連である。
  * 関連端 observation の多重度は ``*`` である。
  * 上記関連のノートを参照。

A_event_timeObservation, A_event_durationObservation
  * それぞれ TimeObservation, DurationObservation から NamedElement への関連である。
  * 関連端 event の多重度は前者が 1 であり、後者は 1 または 2 である。
    かつ後者の event には制約 {ordered} が課せられる。
    「入」「出」の順だろう。
...
* Interval とは値一組で定義される区間、範囲である。
  対象の Element がある与えられた範囲にあることを assert するような
  Constraint で主に用いられる。
...
----------------------------------------------------------------------
Interval
  * ValueSpecification の対に関連する ValueSpecification である。

  TimeInterval, DurationInterval
    * それぞれ TimeExpression, Duration の対に関連する Interval である。

IntervalConstraint
  * 詳細が値の範囲で与えられる Constraint である。

  TimeConstraint, DurationConstraint
    * 制約になる範囲がそれぞれ TimeInterval, DurationInterval である IntervalConstraint である。
    * ともにブーリアン型属性 firstEvent を持つ。
      その多重度は前者が ``0..1`` であり、後者は ``0..2`` である。
      意味は値が true ならば「入」で、そうでなければ「出」である。

A_min_interval, A_max_interval
  * ともに Interval から ValueSpecification への単方向関連である。
  * 関連端 min と max の多重度は 1 であり、
    意味的には min の評価値は max のそれを超えない。
  * 下記の関連はこの関連を redefines したものである。

  A_min_timeInterval, A_max_timeInterval
    * ともに TimeInterval から TimeExpression への単方向関連である。

  A_min_durationInterval, A_max_durationInterval
    * ともに DurationInterval から Duration への単方向関連である。

A_specification_intervalConstraint
  * IntervalConstraint から Interval への composite 関連である。
  * 関連 A_specification_owningConstraint を redefines したものである。
  * 関連端 specification の多重度は 1 である。
  * 以下の関連は、これを redefines したものである。

  A_specification_timeConstraint, A_specification_durationConstraint
    * これらの関連はそれぞれ TimeConstraint から TimeInterval への、
      DurationConstraint から DurationInterval への composite 関連である。
