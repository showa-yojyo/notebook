...
* 前節で見たように structural semantics は対象となるモデルのある瞬間を切り取って表現したものではあるが、
  ...
  * 例えば behavioral feature という概念がある。
    文字通り「振る舞いについての機能」ということ。

  * あるクラスが active であるという概念も振る舞いに関する。

* クラスは method という、その詳細な振る舞いを定義するものを持つことが許されている。

* いくつか重要な用語の定義がポツポツある。

  Operation
    クラスの behavioral feature であり、
    そのクラスのインスタンス上で直接発動 (invoke) されることが許されるもの。
    定義には入力・出力パラメーター、事前条件、事後条件といった概念を含む。

    ひとつの operation はひとつの method を持ってもよい。

    このノートではとりあえず「操作」と訳すことにする。

  Stable
    「何らかの振る舞いが発動している真っ最中ではない」ということを表したいのだろう。
    日本語に訳すとなると、「安定なんとか」のようになる。

  Transient
    「事後条件はメソッドの振る舞いの transient execution の間は成り立つ必要はない」のような使い方をする言葉らしい。
    日本語に訳しにくいが「短期の」「瞬間の」「束の間の」とかそういう意味合い。

  Invariant conditions
    クラスがその操作の前後において必ず成り立つ条件のこと。
    この用語の要点は、その操作の途中は成り立つかどうか問わないところにある。

    このノートでは「不変条件」と訳すことにする。

  Property default values
    これはプログラマーが暗黙のうちに知っている概念と同じとみなして問題なさそうだ。

    * 仕様書中では attributes と properties を微妙に使い分けていることに注意。
...
* テキストを用いる記法の説明では BNF 記法（の変種）を用いる。
  これも記法として潰しが効くので p. 17 をいつでも参照できるようにしておくとよい。

* Notation と Examples の節で現れる図式は色々なツールで制作したものに付き、
...
* 特殊化と再定義。関連端点近くに以下の制約情報表示ラベルを付すことがある。

  ``{subsets endA}``
    「この制約が適用される関連端点は、関連端点 ``endA`` の部分集合である」を意味する。
  ...
  ``{redefines endA}``
    「この制約が適用される関連端点は、関連端点 ``endA`` を再定義する」を意味する。

  ここで関連端点 ``endA`` は既存の関連の要素であることを仮定している。

* 関連端点に多重度が示されていないならば、
  それは多重度がちょうど 1 であることを示唆する。

* 関連端点にラベルが付いていないならば、
  その端点のデフォルトの名前は、アタッチしているクラスの名前から、
  頭文字を小文字に変えたものとする。

  ただ、non-navigable な関連端点にはしばしばラベルを付けないままである
  （名前が Association Descriptions の節に文書化されている場合であっても）。

.. todo:: ここまでの説明を補足するイラストを入れたい。

* 明示的に命名されていない関連については、次のルールに従い「構築」する (p. 19)。
  ここで ``<association-end-nameX>`` はそれぞれの関連端点の名前とする。
