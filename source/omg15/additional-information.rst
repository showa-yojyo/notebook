...
* UML が UML 自身で定義されているということは、
  プログラミング言語の多くがその言語自身で書かれたコンパイラーを持つという事実や、
...
* 次のものは、この意味の差異が UML においてとりわけ重要な領域のいくつかである。
  ...
  Operational behaviors
    ...
    * 操作はメソッドを持つことも許されているが、
      それは要求される振る舞いの詳細な定義である。
  ...
  Property default values
    * 特性の意味は、
      デフォルト値を持つ特性がオブジェクト化されるときに、
      その特性のための固有のセッティングの何らかが不在で、
      デフォルト値が評価されてその特性の初期値を与えることを
      指定するものである。

    * ある classifier をオブジェクト化するときには、
      いったん classifier をオブジェクト化するのが必要な振る舞いのどれもが完了すると、
      そのデフォルト値を持つ属性すべてが適切に初期化されるべきである。
