...
* [1] ツイート ID をパラメーター ``id`` として指定することになっているが、
  PTT では ``_id`` として値を渡すのが無難だ。
...
* [1] ここで示すのは、それほど多くはない決まった数量のツイートを取得する方法となる。
  パラメーター ``count`` に 200 までの値を指定することができる。

  なお 200 を超える量のデータを得たい場合は、パラメーター ``max_id`` をうまく利用すること。
  詳しくは :doc:`./rest` 参照。
