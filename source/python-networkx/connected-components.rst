...
有向グラフの強連結成分の全体と、それらを接続する元のグラフの辺の全体との組で
さらなる有向グラフを考えることがあるためである。これは DAG となっている。
...
.. literalinclude:: ../../sample/networkx/strongly-connected-components.py
   :language: python3
   :lines: 1-13,221-

コード中のグラフ ``G`` はドラクエ 8 の錬金レシピの依存関係を基に構成した（多重）有向グラフだ。
...
これが意味するところは、各連結成分内では、その各アイテム（ノード）に錬金操作に関する推移律が成り立つということだ。
例えば「ふつうのチーズ」から（複数の）錬金操作を経ての「超辛チーズ」の生成可能性を示している。
逆に「超辛チーズ」から「ふつうのチーズ」の生成可能性も示している（ゲームでは勿体ないからやらないほうがよい）。
