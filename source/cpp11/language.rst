...
.. todo::
   ...
   仕様書にある Expression category taxonomy のノートがどこかに要る。
   cpprefjp_ の「右辺値参照・ムーブセマンティクス」の節「式の分類」の元ネタのはず。
...
* 全参照：テンプレートや ``auto`` に ``&&`` をつける場合は意味が異なる。
...
* 完全転送
  ...
  * 引数を全参照で宣言した場合、安易にムーブできない。
    引数は右辺値参照の場合と左辺値参照の場合両方があり得る。
