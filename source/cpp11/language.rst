...
* 全称参照：テンプレートや ``auto`` に ``&&`` をつける場合は上記のときと意味が異なる。
...
* 完全転送
  ...
  * 関数テンプレート ``std::forward()`` は lvalue 参照の場合はムーブせず、
    rvalue 参照の場合はムーブして渡す。

  * さて ``std::forward()`` はスマートな ``std::move()`` であると考えてよいだろうか。
