...
7.1 群の作用と軌道
----------------------------------------------------------------------
* 例 7.1: 双曲線と座標軸で平面を埋める作用 :math:`(x, y) \mapsto (e^tx, e^{-t}y)`

  * 点を固定してパラメーターのように :math:`t \in \mathbb R` を「動かす」と平面上に図形が描かれる。
...
* 例 7.3: 三次元直交群の三次元空間への作用。
  ...
  * 復習。:math:`g \in O_3(\mathbb R)` は等長変換であり、ベクトル間の角度も保存する。
  * 復習。:math:`g \in O_3(\mathbb R)` は（恒等変換に対する）軸回転と平面鏡映の合成の形になっている。

  * :math:`g \in O_3(\mathbb R)` に対して :math:`\det g = \pm 1` なので、
    このうちプラスのほうを集めると部分群 :math:`SO_3(\mathbb R)` が得られる（回転）。
  ...
  * 直交群は次のように分解できる。
    ...
    .. math::
       ...
       \begin{align*}
       O_3(\mathbb{R}) = \mathit{SO}_3(\mathbb{R}) \sqcup h \mathit{SO}_3(\mathbb{R}),\
  ...
  * 軌道を考えると、これは球になる。
    ...
    .. math::
       ...
       \begin{align*}
       \mathbb R^3 = \bigsqcup_{r > 0} S_r \sqcup \{0\},\ 
       S_r = \{x \in \mathbb R^3 \mid \lVert x \rVert = 1\}.
       \end{align*}

* 例 7.4: :math:`\Omega_{\mathbb Z}(\mathbb R),\ \Omega_{\mathbb Z^2}(\mathbb R^2)`

  * 各加法群 :math:`\mathbb{Z}` の各実空間への作用について、軌道と軌道空間を例示している。
...
* 定理 7.8: :math:`Fun(X)^G \longrightarrow Fun(\Omega_G(X))`

  * 前半の主張：自然な写像 :math:`\nu` が well-defined であることを簡単に確認する。
  * 後半の主張：写像 :math:`\nu` が線形同型か。

    * 線形性は問題ない。
    * 全単射性は、これを示すのにある写像 :math:`\mu: Fun(\Omega_G(X)) \longrightarrow Fun(X)^G` を
      写像 :math:`\nu` の逆写像になるように何か定義して、
...
* 例 7.24: 例 7.1 の双曲線作用は :math:`\mathbb R^2` からそれ自身への同相写像だ。
* 演習 7.25: 例 7.9, 7.11 の群が連続群ならば作用は連続だ。

射影 :math:`\pi: X \longrightarrow \Omega_G(X)` を導入する。
...
* 引き戻し
  ...
  * 位相空間 :math:`X` から :math:`Y` への連続写像を :math:`\pi` とする。
    ...
    このとき写像 :math:`\pi^*: C(X) \longrightarrow C(Y)` を
...
* 定理 7.29: 位相空間から商空間への射影による引き戻しは線形同型写像となる。
  ...
  * 証明方針：引き戻し :math:`\pi^*: C(X/G) \longrightarrow C(X)^G` の逆写像を線形同型となるように
