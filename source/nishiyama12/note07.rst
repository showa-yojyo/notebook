...
* 定理 7.8: :math:`Fun(X)^G \to Fun(\Omega_G(X))`

  * 前半の主張：自然な写像 :math:`\nu` が well-defined であることを簡単に確認する。
  * 後半の主張：写像 :math:`\nu` が線形同型か。

    * 線形性は問題ない。
    * 全単射性は、これを示すのにある写像 :math:`\mu: Fun(\Omega_G(X)) \to Fun(X)^G` を
      写像 :math:`\nu` の逆写像になるように何か定義して、
...
射影 :math:`\pi: X \to \Omega_G(X)` を導入する。
...
* 定理 7.29: 位相空間から商空間への射影による引き戻しは線形同型写像となる。
  ...
  * 証明方針：引き戻し :math:`\pi^*: C(X/G) \to C(X)^G` の逆写像を線形同型となるように
