...
* Matplotlib が扱う、図の大きさや図形の色といった、
  ほとんど全てのプロパティーを指定するために
...
* その設定ファイルのあるディレクトリーは、次の優先度で決まる。
  ...
  #. Matplotlib インストール場所から決まるディレクトリー。
     ...
     * 私の場合は :file:`D:/Python35/lib/site-packages/matplotlib/mpl-data/` となる。
     * Matplotlib のアップグレードの度に上書きされると思っていたほうがよい。
       したがって、この 設定ファイルを編集するような状況はまれだと思う。

* 実際に参照される設定ファイルは、関数 :code:`mpl.matplotlib_fname` の呼び出しで確認可能。
...
* PyQt5 が使用可能な環境では試しに :code:`backend : Qt5Agg` としてみる。
