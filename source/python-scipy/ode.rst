...
どうも偏微分方程式は現時点では未実装らしいので、常微分方程式だけを解くことにする。

.. contents::

関数 ``scipy.integrate.odeint``
======================================================================
関数 ``scipy.integrate.odeint`` を利用することで、常微分方程式を数値的に解くことができる場合がある。
...
私の環境での実行結果は次のとおり。いつものような ``print`` ではプログラムの実行内容がさすがに理解不能なので、本稿はプロットによるグラフ描画の出力を示す。
