...
ワーで表示されないことがある。最終結果パスは :abbr:`SVG` パスとして表現される：
   ...
   *Live Path Effects*, or :abbr:`LPEs` for short, is a system for applying some
   kind of effect to a path. Inkscape stores the original path data in the
   Inkscape *Name Space* so that it can be modified at a later time (it will not
   be displayed by other :abbr:`SVG` renderers). If the original path is
   modified, the :abbr:`LPE` will be regenerated. The resulting path is stored
   as a normal :abbr:`SVG` path.

拡張フィルターにあるパス効果を :abbr:`LPE` に移行する計画がある：

   Inkscape includes a handful of :abbr:`LPEs`. With a solid framework in place,
   it is planned to migrate most of the path effects found under the
   :menuselection:`Extensions` menu to :abbr:`LPEs`.

:abbr:`LPE` 使用手順が述べられている：

   To use an :abbr:`LPE`, select a path (the skeleton path), call up the
...
:abbr:`LPE` が生成したオブジェクトを選択し、*Node Tool* がアクティブならば元パス
も表示される：

   When an object is selected that was generated by an :abbr:`LPE` and the *Node
   Tool* is enabled, the original skeleton path nodes are displayed.
...
通常、:abbr:`LPE` はグループ内のオブジェクトすべてに対して再帰的に適用される。し
かし例外が二つは知られている：

   :abbr:`LPEs` are normally applied recursively to all objects in a *Group*.
   The *Bend* and *Deformation* :abbr:`LPEs`, however, are applied to the whole
   *Group* once.

:abbr:`LPE` を通常パスに変換することができる。:menuselection:`Path --> Object to
Path`, :kbd:`Shift` + :kbd:`Ctrl` + :kbd:`C` でいい。

:abbr:`LPE` を消し去るには :menuselection:`Path --> Remove Path Effect` を実行す
る。

:abbr:`LPE` はあるオブジェクトから別のオブジェクトに複製することが可能だ：

   An :abbr:`LPE` can be copied from one object to another using the
   :menuselection:`Path --> Paste Path Effect` (:kbd:`Ctrl` + :kbd:`7`) command.
   To do so, first copy the object with the :abbr:`LPE` you wish to paste using
   the :menuselection:`Edit --> Copy` (:kbd:`Ctrl` + :kbd:`C`) command.

:abbr:`LPE` はある種の不具合が多い：

   The implementation of :abbr:`LPEs` has many bugs, especially if removing
   effects from a path or chaining :abbr:`LPEs`. Paths can be left with
   :abbr:`LPE` cruft in their attributes and without a valid :abbr:`SVG` path.
...
曲線をさらに曲げるコマンドは :abbr:`CAD` でも見かけられる操作だ：
...
======================================================================
   ...
   designed to demonstrate the possibilities of :abbr:`LPEs`. The *Gear*
   extension can also be used to draw gears with a bit more control.
...
しかし、メモリーも :abbr:`CPU` も食う仕様であるようだ：
...
個人的には :abbr:`CAD` の曲面スイープコマンドと仕様が酷似していることに注意した
い。
...
======================================================================
   ...
   *control* or *skeleton* path. The resulting object takes the attributes
   (*Fill*, etc.) of the *skeleton* path.

同名のコマンドが Extensions にもある。本書では :abbr:`LPE` 版を先に紹介している。

   The advantage of using the :abbr:`LPE` version is that both the pattern and
   the *skeleton* path can be edited at a later time. The disadvantages are that
   only paths can be used for the pattern and that there are fewer options.
