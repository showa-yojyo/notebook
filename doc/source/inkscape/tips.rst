...
#. ダウンロードファイルは単なる圧縮ファイルなので（インストーラーではない）、
   使いやすいフォルダーに設置するのみ。
#. 解凍先直下の :file:`inkscape-0.48.5.exe` を実行。
...
#. Inkscape のメニュー項目 :menuselection:`編集 --> Inkscape の設定` を選択す
   る。
#. 設定ウィンドウの左列の下から 3 番目の項目 :guilabel:`インターフェイス` を選択
   する。
#. :guilabel:`言語（再起動が必要）` ドロップダウンリストから :guilabel:`英語
   (en)` を選択する。
...
* :program:`inkview.exe` は SVG ファイルのビューワープログラムである。.svg,
  .svgz, etc. を開ける。
