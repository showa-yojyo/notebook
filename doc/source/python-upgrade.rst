...
* 普通に Explorer 上から実行して、Python 2.6 のインストール場所の隣にインストールする。
...
2.6 環境で :command:`pip freeze` することで、利用中の :file:`site-packages` パッケー
ジが得られる。これを 2.7 環境の :program:`pip` に食わせて様子見とする。
...
* インストーラーを実行する。
  インストールオプションで :program:`pip` が含まれるようにすること。
...
:program:`SendToCygwin` の Windows 7 版が存在しないことを除いて、前述と同様。
...
#. Windows のコントロールパネル「プログラムと機能」から 3.4 のパッケージと
   Python 本体をアンインストールする。
