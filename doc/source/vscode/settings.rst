...
.. |settings.json| replace:: :file:`settings.json`
...
これまで述べてきた設定エディターはファイル |settings.json| に保存されている設定
値の UI だ。このファイルはコマンド :guilabel:`Preferences: Open Settings JSON`
を実行することで、Editor で直接確認・編集する。設定は、設定 ID と値のペアを指定
して、JSON として表現される。

ファイル |settings.json| の編集時には IntelliSense が機能する。設定と値のスマー
ト補完や説明文ポップアップなどが出現し、設定名や JSON の書式が誤っていることによ
るエラーに関するハイライト表示もある。
...
Editor 上で |settings.json| をつねに直接編集したいならば、設定項目
...
ることもできるが、ファイル |settings.json| を開いて中括弧 ``{}`` で囲まれたエン
トリーを削除すれば、変更した設定すべてをリセットできる。ただし、この場合、以前の
設定値を復元できなくなる。
...
ユーザー設定と同様に、ワークスペース設定もファイル |settings.json| に保存され、
コマンド :guilabel:`Preferences: Open Workspace Settings (JSON)` によって直接編
集することができる。
...
ワークスペース設定の |settings.json| はプロジェクトの利用者全員で共有されるもの
だ。
...
ファイル |settings.json| の IntelliSense を利用して、言語固有の設定を見つけられ
る。エディター設定全部と非エディター設定の一部がサポートされている。言語モードに
よっては、:guilabel:`Preferences: Open Default Settings` を実行して表示される
...
.. _anchor-vscode-migration:

移行先での同期操作手順
----------------------------------------------------------------------

Windows PC を新たに購入したなどで、旧 PC における VS Code 環境と同じものを構築す
る手順は次のとおり：

#. VS Code を起動する（初回起動であることが望ましい）
#. Activity Bar の歯車アイコンメニューから :guilabel:`Turn On Settings Sync...`
   を実行する
#. コマンドパレット部分に出現するプロンプトで移行前の環境に合わせて項目を入力する

.. admonition:: 利用者ノート

   この機能を実際に利用したときに期待どおりに動作しなかった点を記す。

   基本的には VS Code 標準の構成項目が維持されるようで、拡張のうち同期されないも
   のがいくつかあった。同期に成功した拡張に共通することを見出してみると、おそら
   くだが、拡張機能の出処が Marketplace であることが同期成功の必要条件なのではな
   いかと考えられる。
